...
% RESULTS
% File: tex/main-study-chapter/chapter6-main-study-RESULTS.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The results from the main study are divided into four sections as follows:
\begin{enumerate}
...
\item \textbf{Section~\ref{main-study:wm-analysis}} focuses on the evaluation component of the study.  The usage of WM is reported, along with participants' positive and negative feedback.
...
All eight participants were active collectors of files, email and bookmarks.  As in the exploratory study, behaviour varied between participants, and between tools for individual participants.  A brief summary is presented as follows.  All were active collectors and organizers of document files (average: 94 folders).  In email, seven participants had large inboxes. Three of these combined a large inbox with extensive filing (F1, F2 and F4).  Participant M6 pursued a frequent-filer strategy.  The remaining four participants, F3, M5, M7, M8, had large inboxes and filed few messages.  On average, participants had  22 email folders.  Bookmarks were indicated to be of low importance by seven of the eight participants who collected them at a very low rate (average: 15 bookmark folders).  There was one exception, participant F4, who collected bookmarks extensively, distorting the overall average of bookmark folders up to 38.
%%%%%%%%%%%%%%%%%%%%%%%%%%
% Cross-tool profiles
%%%%%%%%%%%%%%%%%%%%%%%%%%
Cross-tool profiles were calculated based on participants' tool-specific strategies: CT1 (3 participants), CT2 (2 participants), and CT3 (3 participants).  They are reported along with other profile information in \textbf{Table~\ref{table:main-study:participants}}.
...
Seven of the eight participants were involved in a range of long-term research, teaching and personal activities, reflecting their postgraduate academic careers.  The one exception was participant M6.  He was as an undergraduate had a highly structured set of short-term activities reflecting his degree courses and projects. As in the exploratory study, all participants proved to be very open.
...
Participant F2, cross-tool profile CT1 (F1, E2, B1), was a male PhD student at ICL.  Out of all the participants, he made the most extensive use of WM -- mirrored 35 creation events over the 120 days he had WM installed.  Most of these (21) were related to a major group project called `BOD' (name changed).  His most common mirroring trajectory was from files to email.   Of the 35 events, he identified 2 mistaken mirrorings, and 4 manual mirrorings (made when WM was not operating). He chose to carry on using WM after the closing interview for an additional 80 days.  His primary reason for using WM was to increase the consistency between different folder structures in support of his projects: \textit{``[WM] helps with project management. For example, I started the `BOD' project by creating a folder in email. Straight away, I was prompted whether I want it under documents as well. Therefore it was synchronized from the beginning rather than working in a hotchpotch''}.  He also stated that through the prompting, WM increased his reflection on PIM, resulting in an improved state of organization: \textit{``I think it's forced me to think about my workspace more, and in particular to synchronize between outlook and my files. Before I was not being methodical and I would lose track of things. Now things are more consolidated and structured''}.  Apart from the take-up in mirroring, F2 did not make any significant changes in strategy.  An extract from his WET is provided in \textbf{Table~\ref{table:main-study:wet-example}}.
...
However, approximately 120 days into the study, he made a significant change in his file organizing strategy as part of a New Year resolution ``to be more organized''. In summary, he reorganized his files by moving his active documents to his desktop and applying new structure.  F3's change in strategy is discussed in detail in \textbf{Section~\ref{main-study:results:changes}}.
% Manual mirroring as dual file areas.
% Installation issues: dual docs (NO: after the move)
At that point he also started \textit{manually mirroring} folders, mainly between the Desktop and bookmarks.  He did not use WM since it was not compatible with the Desktop.  Participant F3 thus provide a key example of emergent PIM behaviour which would not be clear from a one-off lab study. 
...
Due to the incompatibility of WM with her email client, Outlook Express, she did not mirror to email, but reported that she could imagine doing so for certain folders such as those related to conference submissions.
% , \textit{``I would mirror these [conference submissions] folders will involve email''}.
% Provide examples of missed mirroring?
In the closing interview, she also reported mirroring 3 of the 15 folders mistakenly (clicking OK without thinking on the WM dialog box), and also missing 3 potentially useful mirroring opportunities.

One limitation of WM that she identified was its focus on one area of the file system -- and she requested the need for multiple file roots in WM on different drives.  However, she also observed that this focus had a key benefit: \textit{``Its good to have everything centralized into one directory. Before I had a problem remembering where to find things. Now its more consistent - and easier to remember.  Previously I'd create a separate folder for each project or conference - but I'd put it anywhere. Mainly on my D-drive but it could be anywhere''}.
...
Participant M5, cross-tool profile CT2 (F2, E2, B2), was a male PhD student at University College London (UCL).  He was one of the few participants to make a change in organizing strategy, and cited the study participation as a key factor in reorganizing his files and email.  This change in strategy is discussed in detail in \textbf{Section~\ref{main-study:results:changes}}.
% CHANGE      
...
Furthermore, he was initially negative towards WM, arguing that he made relatively little use of folders in email and bookmarks.  Over the course of the study he mirrored 5 folders between files and email, and was more positive by the closing interview: ``\textit{Quite impressively, the idea of mirroring structures in emails and in the folders didn't really cross my mind. I think before I had similarly named folders in both systems but they were never agreed in any way. And now I think the structures are a bit more coherent}''.  This represents a second example for why PIM-tools must be evaluated over the long-term. 

% PROMPTIONG UNANTICI
He also acknowledged the increase in reflection offered by WM: ``the act of being prompted ... is useful because it reminds you that you should think about how you should organize your stuff. But its also someone saying ``clean up your room''. However, he interpreted this as a hint to do less PIM: \textit{``When I get the WM message up it reminds me that I'm wasting time with information management, creating folders and putting things in folders!''}.
...
%Good quotes why.
Participant M6 did not respond positively to WM.  % 
%%%%%%%%%%%%%%%%%%%%%
% bm-files partial
%%%%%%%%%%%%%%%%%%%%%
Although he was highly organized in all three tools, he considered his organizational needs to differ between them.  He observed a partial overlap between files and bookmarks: \textit{``Bookmarks and `My Documents' are very much more related to each other.  If you say in Bookmarks, \texttt{my phd work} and then underneath you have \texttt{centra-fusion} and \texttt{cog-robotics} and \texttt{research-labs} ... the first two you would have in files as well, but the third thing you probably wouldn't as you wouldn't put files in there''}.  Although he also organized his email extensively, he only did so in a few top-level folders.  During the study, he only mirrored one folder creation, \texttt{Loreal}, from email to files, despite having no installation problems.
...
At the end of the study, he suggested that mirroring should be one-way from email to files, for top-level folders only: \textit{``In my opinion it is `one to many', or like `one to a folder structure' really. You know ... its always email (one thing) links to one thing in my documents which does not necessarily have to be top-level, it could be above or could be in my documents directly but its just more likely to have another branches underneath''}.  This change in opinion regarding WM usage highlights the benefits of performing evaluation over the long-term.  M6 made no change in strategy over the course of the study.
...
In the closing interview, despite not using WM, M7 indicated that certain users may find it useful: \textit{``if they used the same kind of folder structures in the different applications ... I might recommend it because I'd see it as being useful for them conceptually to mirror the folder structures in the other environments}''.
% other users
...
This section provides an overview of the objective data recorded during stage 4 of the main study.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In total, 62 \textit{folder creation events} were mirrored over the course of stage 4 (not including test mirrorings during the interim interview).  However, the author stresses that the resulting average of 7.8 events per participant masks wide variation in usage of WM -- as indicated in the case studies above.  \textbf{Table~\ref{table:main-study:wm-usage}} provides an overview on an individual basis. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Creates versus deletes and renames
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% During stage 4, participants mirrored an average of 8 folder creations.  
One immediate observation is that all mirrored events related to \textit{folder creations}.  No participants used WM to mirror a folder delete or folder rename event, except to test the WM functionality.  As reported in \textbf{Chapter~\ref{chapter:design}}, the mirroring of renaming events was limited to those within a particular folder.  Renames corresponding to moving folders between parent folders were not supported by WM, and resulted in a warning messages being displayed.  No users complained of this.  However, M3 mirrored 2 folder deletes and 4 folder renames \textit{manually}.  Possible reasons for the bias in favour of creations over deletes and renames are discussed in \textbf{Section~\ref{main-study:discussion:evaluation}}.  The rest of this section focuses on the mirroring of folder creation events.
...
% Impact on folder overlap?
% Types (orgdims) of mirrored folders
Five of the 8 participants mirrored four of more folder creation events during stage 4:
\begin{itemize}

\item Two participants, F2 and F4 made extensive use of WM, mirroring 27 and 15 newly created folders respectively, over an average of 116 days.

\item Participant F1, despite being very positive towards WM, only employed it to mirror 4 folder creation events.  His qualitative feedback indicated disruption due to a change in job as being a primary factor in his low PIM activity.

\item Participant M5 only made limited use of WM. Towards the end of the study, he mirrored 3 newly-created folders with WM, and another 2 manually.

\item After his New Year reorganization, Participant F3 \textit{manually} mirrored 10 folder creation events.  He employed manual mirroring as WM did not support the desired trajectory, between his bookmarks and files on the `Desktop'.
...
\begin{tabular}{|c|c|p{1.9cm}|p{1.9cm}|p{1.7cm}|p{2.5cm}|p{2cm}|p{1.8cm}|p{3cm}|}
\hline
{\bf Participant} & {\bf Cross-tool profile} & {\bf Initial attitude to WM} & {\bf Final attitude to WM} & {\bf Installation problems, see Figure~\ref{table:main-study:participants-problems}} & {\bf Level of WM usage} & {\bf Folder creation events mirrored in stage 4} & {\bf Mirroring trajectory} & {\bf WM usage \underline{with tracking} (\# days)} \\
\hline
        F1 & CT2 (F1, E2, B2) &   Positive &   Positive &            & Extensive before change of job &          4 &    Various &     0 (57) \\
\hline
        F2 & CT1 (F1, E2, B1) &   Positive &   Positive &            & Extensive, ongoing after study &         27 & Mainly from files to email &   71 (120) \\
\hline
        F3 & CT3 (F1, E3, B2) &   Negative &    Neutral &      files & Little use of WM. Started manual mirroring towards end of stage 4 & 10 (8 manual) & Mainly manual from desktop to bookmarks &   93 (143) \\
\hline
        F4 & CT1 (F2, E2, B1) &   Positive &   Positive &      email &  Extensive &         15 & Mainly from bookmarks to files &   72 (111) \\
\hline
        M5 & CT2 (F2, E2, B2) &    Neutral &   Positive & files, email & Some mirroring towards end of stage 4 &          5 & From files to email &         65 \\
\hline
        M6 & CT1 (F1, E1, B1) &   Negative &    Neutral &            &        Low &          1 &            &         18 \\
\hline
        M7 & CT3 (F2, E3, B3) &   Negative &   Negative &      files &       None &          0 &          - &         16 \\
\hline
        M8 & CT3 (F2, E3, B3) &   Positive &   Positive &            &       None &          0 &          - &         20 \\
\hline
{\bf Average} &          - &          - &          - &          - &          - &          8 &          - &         69 \\
\hline
\end{tabular}  
...
% DEPTH and ORG DIM
%%%%%%%%%%%%%%%%%%%%%
Mirrored folders were mostly high up in participants' folder structures, and the average depth was XX.X.  In terms of organizational dimensions, mirrored folders mostly corresponded to \textit{role} (XX\%) and \textit{project} (YY\%).  
...
\hline
{\bf Event source} & {\bf \# mirrored creation events} & {\bf Destination: files} & {\bf Destination: email } & {\bf Destination: BM} & {\bf Destination: files and email} & {\bf Destination: files and BM} & {\bf Destination: email and BM} \\
\hline
{\bf Files} &         40 &          - &         25 &          5 &          - &          - &         10 \\
\hline
{\bf Email} &         20 &         10 &          - &          2 &          - &          8 &          - \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Of the remaining three participants, M6 mirrored one folder from email to files, and M7 and M8 made no use of WM.  However, all participants ran WM on their computers to test its robustness, and provided qualitative feedback.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Overall, there was a marked difference between the track 1 and track 2 participants, who mirrored on average 14 folders, and 1.5 folders respectively.  Possible reasons for this variation are discussed in \textbf{Section~\ref{main-study:discussion:evaluation}}.
...
However, a general theme mentioned by all eight participants was that mirroring should not necessarily be a one-to-one mapping between the three tools due to differing organizational requirements. In many cases, files were seen to be more important for filing than email, e.g. F5: ``\textit{It doesn't make sense to create an email folder for every single publication so I just have a single submissions folder that goes across the publications and has, lets say,  at the moment maybe 30 entries or so. In my H: on the other hand, every single publication is a project  - and deserves its own folder because it consists of much more files than just the five emails. So that's an example where it makes sense that H: and email correspond but are not exactly mirrored}''.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Seven participants suggested that mirroring was particularly appropriate for \textit{top-level folders}, e.g. M8: \textit{``But now I want images related to my project and all the substructure of that project its very difficult to see why you'd want to mirror all that ... once you've mirrored there [at the top-level] - you might not want to mirror it further down. I think it would work at that level. Here are the projects I'm working on.  Here are the emails about that project. And here are web links related to the project. That makes sense to me''}.
...
% CLUTTER
%%%%%%%%%%%%
M7 felt that mirroring would result in clutter due to unused empty folders.  However, the comments from several other participants suggested that they felt the benefits of increased consistency between folder structures outweighed the risk of clutter.  For example, F3 welcomed having folders mirrored and ready for use in the future, even if they were not used immediately, \textit{``I haven't written any latex so I haven't used it, but I'm keeping it.  I think it may be useful''}.  

\textbf{Section~\ref{main-study:results:themes-design-recs}} reports the design suggestions made regarding making mirroring customizable.
...
Participants had a range of views on the prompting from WM.  Three (M5, M6, M7) found it annoying, and suggested possible alternatives.  For example, M5 suggested that a more selective mirroring scheme: \textit{``It is not necessary to always replicate - it should prompt me in sensible cases. Maybe in higher-level folders, and not in lower level folders''}.  M6 noted that it was only a minor problem, \textit{``It kept on popping up and asking me the same silly question ... Well its not really a problem since its just one cancel. Compared to what Word normally does to you, or Office or Windows - all kinds of harassing boxes - its negligible!''}.

Participant F4 highlighted the extra decision-making introduced by WM, which she did not always have time for.  She reported how she often did not have time for decoding whether to mirror or not and just pressed OK, or closed the dialog box.  As a result, she reported a number of missed and mistaken mirrorings.  % \textit{Overhead of prompting.  Participant F4 clicking OK and closing box, and just mirroring to get it out of the way.  In other words, extra overhead is being created.} 
...
% \item Manual (F2, F3)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Three participants (F2, F3, and M5) reported carrying out \textit{manual mirroring}.  F2 did so when WM was not working.  The other two performed manual mirroring due to WM incompatibilities (see \textbf{Table~\ref{table:main-study:participants-problems}}). F3 manually mirrored folders from his bookmark collection to his desktop, and M5 manually mirrored 2 folders from his network drive to email.

%%%%%%%%%%%%%%%%%%%%
% missed mirrors
% \item Missed (F3, F4)
%%%%%%%%%%%%%%%%%%%5
Two participants (F4 and F4) reported occasionally forgetting to mirror some folders which in retrospect could have been usefully done so.  Participant F4 reported \textit{``just closing the WM dialog box''} to save on time. 

%%%%%%%%%%%%%%%%%%%%%%%
% MISTAKEN MIRRORINGS
%%%%%%%%%%%%%%%%%%%%%%%
There were also a number of \textit{mistaken mirrorings} reported by participants F2, F3 and F4.  Again, participant F4 highlighted a lack of time for making mirroring decisions as a key factor.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Participants F3 and M8 noted difficulties in accommodating WM in an existing personal information environment with different file, email and bookmark folder structures.  Participant M8 noted that he would need to perform a significant reorganization of his workspace to make WM worthwhile, and although he planned to do so, had not got round to it over the course of the study. 
% Therefore best if use when setting up. Therefore recommendations for certain types of other people
Participants F3 suggested that WM would be most appropriate to people setting up a new computer, \textit{``I'd say you get you'd get some seriously different results if you installed WM on someone with a brand new computer about to start to using it ... I think you'd get a very different dynamic - and you my might even get a completely different usage out of the same person ... most of my pre-organization had already occurred by then - reasonably quickly after purchasing the computer''}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Although participant M6 was profiled as \textit{pro-organizing} in all three collections, he had very different organizing behaviour in each.  In files he had a deep structure of hundreds of folders, whilst his email only contained a few top-level folders.  He argued for the unidirectional mirroring of top-level folders from email to files only.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Participant M7 was initially unfavourable towards WM, and demanded more powerful integration based on folder-sharing, \textit{``I'd create a folder but it would be accessible from all three - rather than just an extra empty folder in the other two.  So I wouldn't get this massive bloat ... the problem with the mirroring and the way I work is the lots of empty folders ... and that to me would be annoying and would impact how I work''}.
Furthermore, over the course of the study, M7 carried out little foldering, stating that he had instead been working on paper.  % In contrast to the other participants, he also stated that unused folders would not offer consistency, but instead clutter.
...
% M8
%%%%%%%%
The primary reason offered by participant M8 for not using WM, was that he had performed very little PIM on his main work computer, and had instead been using another computer for experimental work.  Although he stated that he could foresee benefits to using WM in the future, he also suggested that he would have to perform significant reorganizing of his files beforehand.  In other words, he foresaw significant overheads in bootstrapping his usage.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
In general, apart from early versions of the prototype occasionally crashing, WM performed well. Participant M7 was the only one to note performance issues: \textit{``It just seems incredibly clunky ... its a proof of concept, but if you do plan on making it more available ... The other thing is I suppose its been intrusive in terms that I've had a few performance-related issues that I've attributed to WM''}.
...
Several participants reported benefits and problems due to WM that had not been anticipated by the researcher.
...
The second unanticipated benefit was reported by participants F2 and F4. They noted that since WM was limited to monitoring one area of the file system they were implicitly encouraged to store more files there to take advantage of the mirroring facility. F4 described this as follows: \textit{``Its good to have everything centralized into one directory. Before I had a problem remembering where to find things.  Now its more consistent, and easier to remember''}. % Good for tracking, makes it easier to remember where things are''}.
...
This section presents the design suggestions made by the participants over the course of the evaluation. All participants were helpful in making suggestions, whether they responded positively or negatively to WM.
...
\item As noted above, seven participants suggested that mirroring should focus on \textit{top-level folders only}, e.g. M5: \textit{``It should not always prompt me ... it should prompt me in sensible cases. Maybe in higher-level folders, and not in lower level folders''}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Three participants expressed the need to \textit{mirror folders between different locations} in different PIM-tools, e.g. M6: \textit{``It would nice if you could browse and say `I want to create the folder at that level'. You know a button `browse' where you can select where you want to create a new one.''}.
% The default being "My Documents" because that'll be the case most of the time

%%%%%%%%%%%%%%%%%%%%%%%
% Mirror names only
%%%%%%%%%%%%%%%%%%%%%%%
\item Two participants suggested a facility to \textit{give mirrored folders different names in different tools} -- Several participants wanted more flexibility regarding mirroring, such as selecting where a folder would be mirrored to, and whether to change its name. % -- i.e. to mirror folder names but allow flexibility in terms of their location.  

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Need for more mirroring flexibility/power
% #1: map to arb existing folders
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Two wanted to be able to \textit{associate a newly-created folder with another already existing folder} in another tool, M6: \textit{``You might have to say `I don't want to create a folder, because that one already exists' ... You might start a project and not have an email folder, but you receive so many emails on it that you create a folder for it later''}.

%%%%%%%%%%%%%%%%%%%%%%%%
% One-way mirroring
% Flex: certain directions only: M6 in the extreme. 
%%%%%%%%%%%%%%%%%%%%%%%%
\item Participant M6 saw the need for a \textit{one-way mirror} from email to files, limited to top-level folders only.
...
%%%%%%%%%%%%%%
Two participants talked about how the mirroring scheme could be made ``more intelligent'', such as automatically mapping a new folder to one with the same name in a different tool,  e.g. F3: \textit{``If I was duplicating a folder that I didn't need to duplicate, like I did there - it could have reminded me. And then I would have gone `ah', instead of creating a new folder and arsing around like that, what I should have been doing was using that one''}.  One participant suggested that folder-mirroring could be combined with a \textit{history-based mechanism} to avoid clutter due to unused folders.  The mechanism would hide unused folders by default, but they would be available to seed folder names in a ``new folder'' dialogue.
...
Participant F1 argued that mirroring could be extended to other PIM-tools local to the desktop, such as photo managers. He also raised the possibility of applying folder-mirroring to the organizational schemes used to manage software applications, such as the ``Start Menu'', the ``Program Files'' folder, and clusters of program icons on the desktop in MS-Windows
\footnote{Interestingly, no participants suggested applying folder mirroring  around the wider personal information environment beyond the local computer: (1) to a user's other computers, (2) to other devices involved in PIM such as PDA devices, and (3) to on-line PIM-tools such as web-mail. In fact, one can also imagine the application of mirroring to ``web portals'' that offer a suite of PIM-tools. For instance, providers such as Yahoo!, Google and MSN offer a range of PIM tools (e.g. email, task list, contacts, document storage, notes). Currently, each PIM-tool enables the management of a distinct collection of personal information which must be separately structured.  These possibilities are discussed in \textbf{Chapter~\ref{chapter:conclusion}}}.

Finally, participant F4 was interested in mirroring folders across users, \textit{F3: ``How about institutional templates?  The department could define starting points for PhD students. A PhD student might want their supervisor's folders.   My Mum might for instance like Elton John's folder structure or Schumacher's!''}.
...
Three participants (F1, F2, and F4) wanted increased support for cross-tool project management.  Two participants wanted a ``cross-tool start-project'' facility, e.g. 6.1: \textit{``Sometimes a project is started after an exchange of emails and so usually you keep the emails in your inbox ... after a while it becomes clear that the project is going to emerge and then what I'll is create a new folder in my email, copy all those emails in there and that gets mirrored to the other things ... that would be good. But sometimes I say we've got this new project which I want to start and I'm going to start it now. This is more or less the same as you get in Visual Studio where you say `new project' and it creates all the necessary folders for you. Maybe you ought to have that on the desktop''.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Cross-tool project support, e.g. archiving
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Two (F1 and F4) were also interested in cross-tool archiving,F1: \textit{``If the project has finished it would be archived or something like that. It would go somewhere - like a central folder like `COMPLETED PROJECTS' where you get all your emails, all your files and all your bookmarks together''}.                        
% F4: actually you've got archive there, but it doesn't really allow you to archive - perhaps does. I haven't used it. It probably could. Again, I wouldn't want them under the active root folder. Would it allow me to do that? 
...
The results from the evaluation that have been presented here are discussed in \textbf{Section~\ref{main-study:discussion:evaluation}}.
